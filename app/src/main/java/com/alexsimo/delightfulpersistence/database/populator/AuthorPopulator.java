...
public class AuthorPopulator {
  ...
  public static void populate(SQLiteDatabase db) {
    db.insert(AuthorModel.TABLE_NAME, null, Author.FACTORY.marshal().name("J. K. Rowling")
        .birth_year(new GregorianCalendar(1965, 7, 31))
        .asContentValues());

    db.insert(AuthorModel.TABLE_NAME, null, Author.FACTORY.marshal().name("Bella Forests")
        .birth_year(new GregorianCalendar(197, 17, 31))
        .asContentValues());

    db.insert(AuthorModel.TABLE_NAME, null, Author.FACTORY.marshal().name("Norah Roberts")
        .birth_year(new GregorianCalendar(1950, 10, 10))
        .asContentValues());

    db.insert(AuthorModel.TABLE_NAME, null, Author.FACTORY.marshal().name("David Baldacci")
        .birth_year(new GregorianCalendar(1960, 8, 5))
        .asContentValues());

    db.insert(AuthorModel.TABLE_NAME, null, Author.FACTORY.marshal().name("Jeff Wheeler")
